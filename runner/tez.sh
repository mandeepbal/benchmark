...
JAVA_HOME="/usr/jdk64/jdk1.6.0_31" ~/apache-maven-3.1.1/bin/mvn clean install -DskipTests=true -Dmaven.javadoc.skip=true

HADOOP_USER_NAME=hdfs hadoop dfs -put tez-dist/target/tez-0.2.0-full/tez-0.2.0-full /apps/

cd ..
echo "<configuration><property><name>tez.lib.uris</name><value>"\${fs.default.name}/apps/tez-0.2.0-SNAPSHOT,\${fs.default.name}/apps/tez-0.2.0-SNAPSHOT/lib/"</value></property></configuration>" > tez-site.xml
